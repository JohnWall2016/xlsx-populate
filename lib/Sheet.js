...
class Sheet {
    ...
    insertAndCopyRow(rowNumber, copyRowNumber, keepValue = false) {
        let copyRow = this._rows[copyRowNumber];
        if (!copyRow) return this.insertRow(rowNumber);
        let { name, attributes } = copyRow._node;
        attributes = Object.assign({}, attributes);
        attributes.r = rowNumber;
        let row = this.insertRow(rowNumber, {
            name, attributes, children: []
        });
        row._cells = [];
        for (let cell of copyRow._cells) {
            if (cell) {
                let newCell = Object.create(Cell.prototype);
                Object.assign(newCell, cell);
                newCell._row = row;
                if (!keepValue) newCell._value = '';
                row._cells[newCell.columnNumber()] = newCell;
            }
        }
